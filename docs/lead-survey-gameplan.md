...
**Status:** live at `https://check.abra-ca-dabra.app/check`. The app is in `readiness-check/` in this repo (copied from Schema commit `c170a2b`) because GitHub denied the push to `makemoney2023/schema`. Written 2026-09-23. Detailed in the design spec [readiness-check/2026-09-23-readiness-check-design.md](readiness-check/2026-09-23-readiness-check-design.md) and the implementation plan [readiness-check/2026-09-23-readiness-check-plan.md](readiness-check/2026-09-23-readiness-check-plan.md); where they differ from this document, the spec wins. The landing and guides answer. Starting a check still returns "Missing Supabase admin env" until that project has the Supabase admin keys. Booking still needs a Cal.com working-session event.
